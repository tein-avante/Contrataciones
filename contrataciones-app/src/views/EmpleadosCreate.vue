...
const router = useRouter() // Para redirigir al usuario después de guardar
...
// Objeto reactivo para almacenar los errores de validación de la API
const errors = ref({})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await axios.post('/empleados', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    toast.success('Empleado creado exitosamente') // <--- TOAST
    // Si la creación es exitosa, redirigimos al usuario a la lista de empleados
    router.push({ name: 'empleados.index' })
  } catch (error) {
    // Si la API devuelve un error 422 (Error de Validación)
    if (error.response && error.response.status === 422) {
      errors.value = error.response.data.errors
      toast.warning('Por favor, revisa los campos del formulario') // <--- AVISO
    } else {
      toast.error('Error al crear el empleado') // <--- ERROR
    }
  }
}
...
<template>
  <AppLayout>
    <div class="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
      <h1 class="text-2xl font-bold mb-6 text-gray-800 border-b pb-4">Añadir Nuevo Empleado</h1>
      <form @submit.prevent="handleSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="md:col-span-2 bg-indigo-50 p-4 rounded-lg border border-indigo-100 mb-4">
            ...
            />
            <p class="text-xs text-gray-500 mt-1">Sube una foto de frente para el expediente digital. (JPG, PNG)</p>
        ...
        <div class="mt-8 flex justify-end items-center border-t pt-6">
          <router-link to="/empleados" class="text-gray-600 hover:text-gray-800 mr-4"
            >Cancelar</router-link
          >
          <button
            type="submit"
            class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg shadow transition duration-150"
          >
            Guardar Empleado
          </button>
        </div>
